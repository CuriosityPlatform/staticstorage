...
FROM base as make-staticstorage
...
FROM scratch as staticstorage-out
COPY --from=make-staticstorage /out/* .
...
FROM debian:10 as make-staticstorage-image

RUN apt-get update && apt-get install -y ca-certificates && apt-get clean

COPY --from=staticstorage-out staticstorage /app/bin/

ENTRYPOINT ["/app/bin/staticstorage"]